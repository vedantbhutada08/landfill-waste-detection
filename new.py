...
non_biodegradable_classes = ['alu_foil', 'cat_food_cans', 'clear_plastic', 'food_cans', 'glass', 'hdpe_clear', 'hdpe_color', 'pet_clear-blue', 'pet_coloured', 'polypropelene', 'polystyrene_-Food trays-', 'polystyrene_-other-', 'tetra-pak', 'thermoforms', 'universal_beverage_cans', 'wte_-non-recyclable_plastic-']

# Load YOLO model with the "best.pt" file
model = YOLO(r"C:\Users\vedan\OneDrive\Desktop\visem_objectdetection\best (1).pt")
...
def process_image(image_path):
    ...
    if results:
        ...
        for box in results[0].boxes:
            ...
            rect = patches.Rectangle((x_min, y_min), width, height, linewidth=1, edgecolor=border_color, facecolor='none')
            ax.add_patch(rect)
...
def display_info(biodegradable_count, non_biodegradable_count):
    ...
    biodegradable_label = ttk.Label(info_window, text="Biodegradable Waste", style='My.TLabel')
    biodegradable_label.pack()

    biodegradable_info_label = ttk.Label(info_window, text=biodegradable_info, justify=tk.LEFT, wraplength=400, padx=10)
    biodegradable_info_label.pack()

    non_biodegradable_label = ttk.Label(info_window, text="Non-biodegradable Waste", style='My.TLabel')
    non_biodegradable_label.pack()

    non_biodegradable_info_label = ttk.Label(info_window, text=non_biodegradable_info, justify=tk.LEFT, wraplength=400, padx=10)
...
image_path = r"C:\Users\vedan\OneDrive\Desktop\visem_objectdetection\74508e6d9dc9d69c2befb4fbfccf894b.jpg"  # Provide the path to your image
image = Image.open(image_path)
